...
#ifdef OS_TYPE_UNIX
    extern Void G_sigIntr(int);
    extern Void G_sigPipe(int);
    extern Void G_sigTerm(int);
    extern Void G_sigKill(int);
#endif
...
main(int argc, char *argv[])
{
    ...
    signal(SIGKILL, G_sigKill);
      ...
      while ((c = getopt(argc, argv, "vVp:T:")) != EOF)
	  if (c == 'v' || c == 'V') {
      ...
      printf ("%s, %d: before CONFIG init\n", __FILE__, __LINE__);
      if ((rc = CONFIG_init()) != Success)
      {
	  sprintf(globals.tmpbuf, "%s: Could not initialize CONFIG module\n", __applicationName);
	  EH_fatal(globals.tmpbuf);
      }
      printf ("%s, %d: after CONFIG init\n", __FILE__, __LINE__);
	  ...
	  sprintf(globals.tmpbuf,
		  "%s: Could not open configuration file (%.*s), reason 0x%04x\n",
		  __applicationName,
	  ...
	  if ((rc = CONFIG_getString(globals.hConfig, "Trace", buf, &p)) == Success)
	  {
      ...
      optind = 1;
      while ((c = getopt(argc, argv, "vVT:")) != EOF)
      {
	  switch (c)
	  {
	  case 'T':
	  ...
	  default:
	    usage:
	      sprintf(globals.tmpbuf,
		      "usage: %s [-V] [-T <TM option string>] <configuration file name>\n",
		      __applicationName);
	      EH_fatal(globals.tmpbuf);
	      break;
	  }
      ...
      TM_TRACE((globals.hTM, MTS_TRACE_INIT, "MTA build date: %s  %s", __TIME__, __DATE__));
	  ...
	  if ((rc = CONFIG_getString(globals.hConfig, "EMSD Hosts", buf, &p)) == Success)
	  {
	      ...
	      TM_TRACE((globals.hTM, MTS_TRACE_INIT, "EMSD Host: %s", pHost->pHostName));
	  }
      };

#ifdef OLD_PROFILE      
      /* Open the profile */
      rc = PROFILE_open(globals.config.pSubscriberProfileFile, &globals.hProfile);
      if (rc != Success)
      {
	  sprintf(globals.tmpbuf,
		  "%s: Could not open Originator/Recipient Profile %s; reason 0x%04x",
		  __applicationName,
      ...
      TMR_init(0, 1000);		/* no preallocation; 1000 ms / tick */
      
      /* Initialize the SCH module */
      SCH_init(100);		/* arbitrary maximum number of tasks */
      ...
      G_duMainPool = DU_buildPool(4096, 200, 500);
      ...
      rc = OS_dirOpen(globals.config.deliver.pNewMessageDir,
		      &globals.config.deliver.hNewMessageDir);
      if (rc != Success)
      {
	  EH_fatal("Could not open delivery new-message directory\n");
      }
      
      /* make sure the spool directories are writable */
      if (access (globals.config.deliver.pNewMessageDir, W_OK)) {
	  TM_TRACE((globals.hTM, MTS_TRACE_ERROR, "Permission problem (write): '%s'",
		    globals.config.deliver.pNewMessageDir));
	...
	if (errno != EEXIST) {
	  TM_TRACE((globals.hTM, MTS_TRACE_ERROR, "Cannot create delivery data directory: '%s'", dirPath));
	}
      }
	...
	if (errno != EEXIST) {
      ...
      rc = mts_buildTimerQueue(globals.config.deliver.pSpoolDir, mts_tryDelivery);
      if (rc != Success)
      {
	  sprintf(globals.tmpbuf,
      ...
      rc = mts_buildTimerQueue(globals.config.submit.pSpoolDir, mts_tryRfc822Submit);
      if (rc != Success)
      {
	  sprintf(globals.tmpbuf,
      ...
      rc = TMR_start(TMR_SECONDS(1), NULL, NULL, mts_spoolToDevice, &hTimer);
      if (rc != Success)
      {
	  EH_fatal("Could not start timer to process; spooling to device\n");
      }
      
      /* Create a timer to process Module Management events */
      if ((rc = TMR_start(TMR_SECONDS(1), NULL, NULL, runMM, &hTimer)) != Success)
      {
	  EH_fatal("Could not start timer to process module management events\n");
      }
      
      /* Start clock interrupts */
      if (TMR_startClockInterrupt(TMR_SECONDS(5)) != Success)
      {
	  sprintf(globals.tmpbuf, "%s: Could not start clock interrupt\n", __applicationName);
	  EH_fatal(globals.tmpbuf);
      }
	  ...
	  pass++;
	  SCH_run();
	  TM_TRACE((globals.hTM, MTS_TRACE_SCHED, "Scheduler ran, took %d seconds (+/- 1s)",
		    time(NULL) - lastTime));
      }
      ...
      void * hUserData2)
{
    ...
    {
	sprintf(globals.tmpbuf, "Could not start timer to process module management events\n");
	EH_fatal(globals.tmpbuf);
    }
...
dirFilter(const char * pName)
{
    /* Include only files which end with "C_xxxxxx" */
    ...
    switch (numFields)
    {
	  ...
	  *pNum = highOrder;
	  TM_TRACE((globals.hTM, MTS_TRACE_ADDRESS,
        ...
        default:
	  TM_TRACE((globals.hTM, MTS_TRACE_ADDRESS, 
		    "Error converting EMSD address string to numeric: %s", pStr));
	  rc = Fail;
    }
    return (rc);
}
