		   ...
		   void * hShellReference)
{
    ...
    /* Update the submission status; no longer pending */
    pMessage->status.hSubmission = NULL;

    /* Begin adding to this transaction */
    if ((rc = NVM_addToTrans(hTrans, &hTip)) != Success)
    {
	TM_TRACE((uash_globals.hTM, UASH_TRACE_ERROR,
		  "Could not add message id, reason 0x%x", rc));
	return;
    }

    /* Allocate memory for, and copy, the message id */
    if ((rc = NVM_allocStringCopy(hTip, pMessageId,
				  &pMessage->hMessageId)) != Success)
    {
	TM_TRACE((uash_globals.hTM, UASH_TRACE_ERROR,
		  "Could not copy message id, reason 0x%x", rc));
	return;
    }

    /* End the add-to transaction */
    if ((rc = NVM_endTrans(hTip)) != Success)
    {
	TM_TRACE((uash_globals.hTM, UASH_TRACE_ERROR,
		  "Could not end adding message id, reason 0x%x", rc));
	return;
    }
}
		...
		void * hShellReference)
{
    ...
    /* Create a failure string */
    if (bTransient)
    {
	p = "Submission Failed (Transient)";
    }
    else
    {
	p = "Submission Failed (Permanent)";
    }

    /* Allocate memory for, and copy, the message id */
    if ((rc = NVM_allocStringCopy(hTip, p,
				  &pMessage->hPrintableStatus)) != Success)
    {
	TM_TRACE((uash_globals.hTM, UASH_TRACE_ERROR,
		  "Could not copy message status, reason 0x%x", rc));
	return;
    }
