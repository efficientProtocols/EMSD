...
{
    PineFlags_Seen	= (1 << 0),
    PineFlags_Deleted	= (1 << 1),
    PineFlags_Flagged	= (1 << 2),
    PineFlags_Answered	= (1 << 3)
};
...
typedef struct Message
{
    ...
    /* Flags (for PINE use) */
    unsigned long	flags;		/* PineFlags values */
...
typedef enum {FreeText, Address, Fcc, Attachment} FieldType;
...
typedef struct pine_field {
    ...
    int       noedit;			/* don't edit this field */
    struct headerentry *he;		/* he that goes with this field,
					   a pointer into headerentry array */
...
/*
 * Reserved space for a submission status string.  Initially, when Pine
 * submits the message, this much space is allocated but will contain no data.
 * When each submission attempt is made, a message of the following form will
 * be placed into the "hPrintableStatus" field of the message:
 *
 *         Send attempts: %-4lu  Last failure at: Wed Jun 30 21:49:08 1993
 */
#define	UASH_SUBMISSION_STATUS_SIZE	70
