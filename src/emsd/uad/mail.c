	   ...
	   char * pLocalDomainAndHost)
{
    ...
    if ((pStream = OS_alloc(sizeof(MAILSTREAM) + messageSize + 1)) == NULL)
    {
	return NULL;
    }
    ...
    if (OS_fileRead(pStream->pHeaderText,
		    1, messageSize, hMailFile) != messageSize)
    {
	/* read error */
    ...
    pStream->pHeaderText[messageSize] = '\0';

    /*
     * Find the beginning of the body text, identified by two
     * consecutive newlines.
     */
    if ((pStream->pBodyText =
	 OS_findSubstring(pStream->pHeaderText, "\n\n")) == NULL)
    {
	...
	*++pStream->pBodyText = '\0';
    ...
    rfc822_parse_msg(&pStream->pEnv, &pStream->pBody,
		     pStream->pHeaderText, strlen(pStream->pHeaderText),
		     &bodyString, pLocalDomainAndHost, tmp);
...
mail_free_envelope(ENVELOPE ** ppEnv)
{
	...
	if ((*ppEnv)->remail)
	{
	    OS_free((*ppEnv)->remail);
	}
	
	mail_free_address (&(*ppEnv)->return_path);
	...
	mail_free_address(&(*ppEnv)->from);
	mail_free_address(&(*ppEnv)->sender);
	mail_free_address(&(*ppEnv)->reply_to);

	if ((*ppEnv)->subject)
	{
	    OS_free((*ppEnv)->subject);
	}

	mail_free_address(&(*ppEnv)->to);
	mail_free_address(&(*ppEnv)->cc);
	mail_free_address(&(*ppEnv)->bcc);

	if ((*ppEnv)->in_reply_to)
	{
	    OS_free((*ppEnv)->in_reply_to);
	}
	
	if ((*ppEnv)->message_id)
	{
	    OS_free((*ppEnv)->message_id);
	}

	if ((*ppEnv)->newsgroups)
	{
	    OS_free((*ppEnv)->newsgroups);
	}

	OS_free(*ppEnv);		/* return envelope to free storage */
    }
